...
def get_tics(pth):
    ...
    with open(pth, mode='r') as fobj:
        dic = {}
        for line in fobj:
            exchange, tic = line.lower().strip().replace('\"','').split('=')
            dic[tic] = exchange
        return dic
...
def line_to_dict(line):
    ...
    i = 0
    for col in COLUMNS:
        dic[col] = line[i: i+COLWIDTHS[col]]
        i += COLWIDTHS[col]
    return dic
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if tickers_lst is None:
        ...
        for i in tickers_lst:
            if i not in list(tic_exchange_dic.keys()):
                raise Exception(f"this ticker {i} does not correspond to a key tic_exchange_dic.")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        tic_dic = {}
        tic_dic['exchange'] = tic_exchange_dic[tic]
        ...
        for line in lines:
            prc_info = {}
            for col in col_lst:
                prc_info[col] = line_to_dict(line)[col]
            data.append(prc_info)
